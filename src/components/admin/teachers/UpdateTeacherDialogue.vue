    ...
    <v-dialog persistent v-model="UpdateDialog" width="400">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn @click="UpdateTeacher()" color="blue darken-1" text>
            OK
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default {
  ...
  props: ["selectedEmail", "selectedName", "id", "UpdateDialog"],
  async mounted() {
    if (this.id) {
  ...
  methods: {
    ...
    async UpdateTeacher() {
      try {
        ...
        await axios.put("http://127.0.0.1:5000/api/users/" + this.id, updatedTeacher);
        this.$toasted.show("Teacher successfully updated!");        
      } catch (error) {
        console.log(error);
      }
      this.$emit("closeUpdateDialog")
